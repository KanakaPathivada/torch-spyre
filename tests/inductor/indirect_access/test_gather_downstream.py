...
class TestGatherFusedDownstreamOperations:
    ...
    def test_gather_3d_unary(self, op, diff_key):
        ...
        compare_with_cpu(lambda x, i: op(x[i]), x, idx, atol=_ATOL_F16, rtol=_ATOL_F16)

    def test_gather_sqrt_bfloat16(self):
        ...
        compare_with_cpu(
            lambda x, i: torch.sqrt(x[i]), x, idx, atol=_ATOL_BF16, rtol=_ATOL_BF16
        )

    def test_gather_exp_float32(self):
    ...
    def test_gather_triple_unary(self):
    ...
    def test_gather_scalar_sub(self):
        """Scalar subtraction (- 0.5) after gather."""
        x = cached_randn((32, 128), differentiation="gds07", dtype=torch.float16)
        idx = torch.randint(0, 32, (16,), dtype=torch.int32)
        compare_with_cpu(
            lambda x, i: x[i] - 0.5, x, idx, atol=_ATOL_F16, rtol=_ATOL_F16
        )

    def test_gather_scalar_div(self):
    ...
    def test_gather_mean_reduction(self):
        """mean(dim=1) after gather; output collapses to (16,)."""
        x = cached_randn((32, 128), differentiation="gds09", dtype=torch.float16)
        idx = torch.randint(0, 32, (16,), dtype=torch.int32)
        compare_with_cpu(
            lambda x, i: x[i].mean(dim=1), x, idx, atol=_ATOL_F16, rtol=_ATOL_F16
        )

    def test_gather_sum_dim0(self):
        """sum(dim=0) after gather; output shape (128,)."""
        x = cached_randn((32, 128), differentiation="gds10", dtype=torch.float16)
        idx = torch.randint(0, 32, (16,), dtype=torch.int32)
        compare_with_cpu(
            lambda x, i: x[i].sum(dim=0), x, idx, atol=_ATOL_F16, rtol=_ATOL_F16
        )

    def test_gather_cos_cpu_fallback(self):
    ...
    def test_gather_log_cpu_fallback(self):
    ...
    def test_gather_abs_then_sum(self):
    ...
    def test_named_kv_pattern(self):
    ...
    def test_named_full_attention(self):
    ...
    def test_named_dims_propagate(self):
    ...
    def test_named_with_downstream(self):
    ...
    def test_sencores_2d(self, sencores, diff_key):
    ...
    def test_sencores_32_embedding(self):
    ...
    def test_sencores_32_paged_kv(self):
    ...
    def test_gate_off_no_gather_op(self):
    ...
    def test_gate_on_gather_op(self):
    ...
    def test_lx_planning_enabled(self):
    ...
    def test_compile_cache_hit_with_downstream(self):
    ...
    def test_gather_then_add_broadcast_bias(self):
    ...
    def test_gather_then_mul_and_add_residual(self):
