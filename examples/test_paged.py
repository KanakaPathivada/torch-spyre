...
def paged_spyre(
    ...
    with spyre_hint(tiles={"B": B // 2}):
        with spyre_hint(tiles={"Lq": Lq // q_block_size}):
            with spyre_hint(tiles={"H": H // 4}):
                # queries_tile = queries[tile_b, tile_lq, tile_h, :]
                # real_max_tile = real_max[tile_b, tile_h, tile_lq]
                # denominator_tile = denominator[tile_b, tile_h, tile_lq]
                # output_tile = output[tile_b, tile_h, tile_lq, :]

                with spyre_hint(tiles={"Lk": Lk // kv_block_size}):
                    # slots_tile = block_table[tile_b, tile_lk]
